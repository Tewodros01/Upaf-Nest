...
export default function TransferMoney() {
  ...
  const syncFromUser = useWalletStore((s) => s.syncFromUser);
  ...
  const [selected, setSelected] = useState<TransferUser | null>(null);
  ...
  if (step === 4)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-5" style={{ background: "var(--bg)" }}>
    ...
    return (
      <div className="min-h-screen flex flex-col" style={{ background: "var(--bg)", color: "var(--text)" }}>
        <PageHeader
          ...
          center={
            <span className="text-base font-black">Confirm Transfer</span>
          }
        />
        <div className="flex flex-col gap-5 px-5 py-6">
          <div className="bg-white/4 border border-white/7 rounded-2xl p-5 flex items-center gap-4">
            {selected.avatar ? (
              <img
                src={selected.avatar}
                alt=""
                className="w-14 h-14 rounded-full object-cover ring-2 ring-cyan-500/30 shrink-0"
              />
            ) : (
              <div className="w-14 h-14 rounded-full bg-cyan-500/20 ring-2 ring-cyan-500/30 shrink-0 flex items-center justify-center text-cyan-400 font-black text-xl">
          ...
          <div className="bg-white/4 border border-white/7 rounded-2xl p-5 flex flex-col gap-3">
            ...
            {[
              {
                label: "Amount",
                value: `${parsed.toLocaleString()} coins`,
                color: "text-cyan-400",
              },
    ...
    return (
      <div className="min-h-screen flex flex-col" style={{ background: "var(--bg)", color: "var(--text)" }}>
        <PageHeader
          ...
          center={
            <span className="text-base font-black">Select Recipient</span>
          }
        />
        <div className="flex flex-col gap-4 px-5 py-5">
          ...
          <div className="bg-white/4 border border-white/7 rounded-2xl overflow-hidden">
            {loadingUsers ? (
              ...
              filtered.map((u: TransferUser, i: number) => (
                <button
                  key={u.id}
                  type="button"
                  onClick={() => {
                ...
                >
                  {u.avatar ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-cyan-500/20 shrink-0 flex items-center justify-center text-cyan-400 font-black">
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      <div className="flex flex-col gap-6 px-5 py-6">
        ...
        <div className="bg-white/4 border border-white/7 rounded-2xl p-5 flex flex-col gap-4">
          ...
          {amountErr && (
            <p className="text-xs text-rose-400 font-semibold">{amountErr}</p>
          )}
