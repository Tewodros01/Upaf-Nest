...
import { useMe, userKeys, useUpdateMe } from "./hooks";
      ...
      <div className="flex flex-col gap-6 px-5 py-6">
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="relative">
            {getPublicAssetUrl(avatarPreview ?? me?.avatar) ? (
              <img
                src={getPublicAssetUrl(avatarPreview ?? me?.avatar) ?? undefined}
                alt="avatar"
                className="w-24 h-24 rounded-full object-cover ring-4 ring-emerald-500/40"
              />
            ) : (
              <div className="w-24 h-24 rounded-full bg-white/8 ring-4 ring-emerald-500/40 flex items-center justify-center text-white text-2xl font-black">
        ...
        <div className="flex flex-col gap-4">
          ...
          {(me?.role === "AGENT" || me?.role === "ADMIN") && (
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/8 px-4 py-3 text-xs text-emerald-200">
              Manage Telebirr, M-Pesa, CBE Birr, and bank accounts from the
              Agent Panel.
            </div>
          )}
        </div>
