    ...
    <div className="bg-white/4 border border-white/7 rounded-2xl p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          {getPublicAssetUrl(item.user?.avatar) ? (
            <img
              src={getPublicAssetUrl(item.user?.avatar) ?? undefined}
              alt={item.user?.username}
              className="w-9 h-9 rounded-full object-cover ring-2 ring-white/10 shrink-0"
            />
          ) : (
            <div className="w-9 h-9 rounded-full bg-white/8 ring-2 ring-white/10 shrink-0 flex items-center justify-center text-white text-xs font-black">
              {getAvatarInitials(
                item.user?.firstName,
                item.user?.lastName,
                "?",
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        {type === "withdrawal" && item.accountNumber && (
          <div className="bg-white/4 rounded-xl px-3 py-2.5 col-span-2">
            <p className="text-[9px] text-gray-500 uppercase tracking-wide mb-0.5">
              Account
            </p>
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--bg)" }}>
      ...
      <div className="sticky top-0 z-40 flex items-center gap-3 px-5 py-4 bg-gray-950/90 backdrop-blur-xl border-b border-white/5">
        ...
        <div className="w-8 h-8 rounded-xl bg-linear-to-br from-emerald-400 to-teal-500 flex items-center justify-center shadow-[0_0_12px_rgba(16,185,129,0.4)]">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="relative overflow-hidden px-5 pt-6 pb-4">
          <div className="absolute inset-0 bg-linear-to-br from-emerald-600/15 via-teal-600/8 to-transparent" />
          ...
          <div className="relative flex items-center gap-4">
            <div className="relative shrink-0">
              {getPublicAssetUrl(agent?.avatar) ? (
                ...
                <div className="w-16 h-16 rounded-2xl bg-white/8 ring-2 ring-emerald-500/40 shadow-[0_0_20px_rgba(16,185,129,0.2)] flex items-center justify-center text-white text-lg font-black">
                  {getAvatarInitials(agent?.firstName, agent?.lastName, "?")}
                </div>
              )}
              <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-lg bg-linear-to-br from-emerald-400 to-teal-500 flex items-center justify-center border-2 border-gray-950">
          ...
          {/* Invite code */}
          {invite && (
            <div className="relative mt-4 bg-white/5 border border-white/8 rounded-2xl px-4 py-3 flex items-center justify-between">
              <div>
          ...
          {tabs.map(({ id: tid, label, badge }) => (
            <button
              ...
              className={`relative flex-shrink-0 px-4 py-2 rounded-xl text-xs font-black transition-all ${
                tab === tid
                  ? "bg-linear-to-r from-emerald-500 to-teal-500 text-white shadow-[0_0_16px_rgba(16,185,129,0.4)]"
                  ...
                  return (
                    <div key={s} className="mb-3 last:mb-0">
                      ...
                      <div className="h-1.5 bg-white/6 rounded-full overflow-hidden">
                        <div
                          className={`h-full bg-linear-to-r ${bar} rounded-full transition-all duration-700`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
              ...
              {invited.length === 0 ? (
                ...
                invited.map((u: User) => {
                  ...
                  const totalDep = uDeps
                    .filter(
                      (d: AgentStatsTransaction) => d.status === "COMPLETED",
                    )
                    .reduce(
                      (s: number, d: AgentStatsTransaction) =>
                        s + Number(d.amount),
                      0,
                    );
                  const totalWit = uWits
                    .filter(
                      (w: AgentStatsTransaction) => w.status === "COMPLETED",
                    )
                    .reduce(
                      (s: number, w: AgentStatsTransaction) =>
                        s + Number(w.amount),
                      0,
                    );
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        {getPublicAssetUrl(u.avatar) ? (
                          <img
                            src={getPublicAssetUrl(u.avatar) ?? undefined}
                            alt={u.username}
                          ...
                          <div className="w-10 h-10 rounded-full bg-white/8 ring-2 ring-white/10 shrink-0 flex items-center justify-center text-white text-xs font-black">
                            {getAvatarInitials(u.firstName, u.lastName, "?")}
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
