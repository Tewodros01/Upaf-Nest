...
import CompetitionDetail from "./features/challenge/CompetitionDetail";
...
const App = () => {
  ...
  if (isBootstrappingTelegram) {
    return (
      <div className="min-h-dvh flex items-center justify-center px-6" style={{ background: "var(--bg)", color: "var(--text)" }}>
        <div className="w-full max-w-sm rounded-3xl p-6 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path={APP_ROUTES.learn} element={<RequireAuth><Learn /></RequireAuth>} />
        <Route path={APP_ROUTES.build} element={<RequireAuth><Build /></RequireAuth>} />
        <Route path={APP_ROUTES.challenge} element={<RequireAuth><Challenge /></RequireAuth>} />
        <Route path="/competitions/:id" element={<RequireAuth><CompetitionDetail /></RequireAuth>} />
        <Route path={APP_ROUTES.mentors} element={<RequireAuth><Mentors /></RequireAuth>} />
        <Route path={APP_ROUTES.jobs} element={<RequireAuth><Jobs /></RequireAuth>} />
        <Route
